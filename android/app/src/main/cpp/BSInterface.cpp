...
void BSWriteMixdownFlac(std::function<void(std::string)> writeMixdownCallback) {
    ...
    writeMixdownCallback(flacFile.getFullPathName().toStdString());
...
//    engine->audioDeviceManager.setAudioDeviceSetup(setup, false);
    ...
    if (engine->audioDeviceManager.getCurrentAudioDevice()) {
        ...
        auto dir = juce::File::getSpecialLocation(juce::File::tempDirectory).getChildFile(
                "BSAudioEngineTemp");
        ...
        if (dir.exists() && dir.isDirectory()) {
            for (int i = 0; i < engine->trackArray.size(); i++) {
                for (int j = 0; j < engine->trackArray.getReference(i)->getTakesSize(); j++) {
                    ...
                    juce::AudioBuffer<float> *bufferPtr = engine->trackArray.getReference(
                            i)->getTakeBufferPointer(j);
...
void BSRecordTrack(long startAtMicros, int currentTrackNumber, int currentTakeNumber,
                   std::function<void(long, int, std::string,
                                      std::vector<float>)> recordingStoppedCallback) {
    if (!engine->recordTrack &&
        currentTakeNumber <= engine->trackArray.getReference(currentTrackNumber)->getTakesSize()) {

        if (currentTakeNumber + 1 >
            engine->trackArray.getReference(currentTrackNumber)->getTakesSize())
            engine->trackArray.getReference(currentTrackNumber)->newTake();

        engine->trackArray.getReference(currentTrackNumber)->setCurrentTakeNumber(
                currentTakeNumber);
        ...
        engine->recordingStoppedCallback = recordingStoppedCallback;
        if (startAtMicros > 0)
...
void BSPlayTrack(long startAtMicros, int currentTrackNumber, int currentTakeNumber,
                 std::function<void()> playTrackFinishedCallback) {
    engine->engineStopped = false;
    engine->metronomeModulo = microsToSamples(startAtMicros) % engine->samplesPerBeat;
    engine->playTrackFinishedCallback = playTrackFinishedCallback;
...
void BSClearAllUnusedTrackTakes() {
    for (int trackNum = 0; trackNum < engine->trackArray.size(); trackNum++) {
        ...
        juce::File selectedFile = flacFilePathFromTrackNumber(trackNum,
                                                              track->getCurrentTakeNumber());
...
void BSMultitrackPlay(long startAtMicros, std::function<void()> multitrackPlayFinishedCallback) {
    ...
    engine->multitrackPlayFinishedCallback = multitrackPlayFinishedCallback;
    engine->setOutputBufferPos(microsToSamples(startAtMicros));
}

void BSMultitrackRecord(long startAtMicros, int currentTrackNumber, int currentTakeNumber,
                        std::function<void(long, int, std::string,
                                           std::vector<float>)> recordingStoppedCallback) {
    if (!engine->recordTrack &&
        currentTakeNumber <= engine->trackArray.getReference(currentTrackNumber)->getTakesSize()) {
...
void BSSetTrackPan(int track, float pan) {
    engine->trackArray.getReference(track)->panChannelGainArray.getReference(
            0) = channelGainFromPan(pan, 0);
    engine->trackArray.getReference(track)->panChannelGainArray.getReference(
            1) = channelGainFromPan(pan, 1);
}
...
void
BSAnalyseTrack1(std::string absoluteFilePath, void recordingStoppedCallback(std::string filePath)) {

}

void BSAnalyseTrack(std::string absoluteFilePath,
                    std::function<void(long, int, int, long)> analyseTrackCallback) {
    juce::File file(absoluteFilePath);
    if (file.exists()) {
        juce::FlacAudioFormat flacFormat;
        juce::AudioFormatReader *reader = flacFormat.createReaderFor(
        ...
        analyseTrackCallback(lengthInSamples, sampleRate, bitdepth, lengthInMicros);
    }
}
...
void BSCompressFiles(std::function<void(std::vector<std::string>)> compressFilesCallback) {
    ...
    compressFilesCallback(compressedFilepaths);
}
...
void BSCompressFile(std::string absoluteFilePath, std::function<void(std::string)> compressFileCallback) {
    ...
    if (file.exists()) {
        ...
        juce::File flacFile = engine->tempDirectory.getChildFile(legalFilename);
        writeCompressedFlacFile(flacFile, bufferPtr);
        compressFileCallback(flacFile.getFullPathName().toStdString());
    }
}

void BSReadFile(std::string filePath, int trackNum, int takeNum, bool sampleLoop,
                std::function<void(long, int, std::string,
                                   std::vector<float>)> readFileCallback ) {
    juce::File file = juce::String(filePath);
    juce::AudioBuffer<float> *bufferPtr = engine->trackArray.getReference(
            trackNum)->getTakeBufferPointer(takeNum);
    readFile(file, bufferPtr);
    engine->trackArray.getReference(trackNum)->setSampleLoop(sampleLoop);

    // callback info
    int sampleRate = engine->sampleRate;
    std::string newFilePath = flacFilePathFromTrackNumber(trackNum,
                                                          takeNum).getFullPathName().toStdString();
    ...
    if (file.exists() && bufferPtr->getNumSamples() > 0) {
        ...
        std::vector<float> waveform = waveformArray(bufferPtr);
         readFileCallback(recLen, sampleRate, newFilePath, waveform);
    } else {
        ...
        std::vector<float> emptyVector = {0.0f, 0.9f, 1.0f};
         readFileCallback(recLen, sampleRate, newFilePath, emptyVector);
    }
}
...
void BSSetCountInBars(int bars, std::function<void()>countInFinished) {
    if (bars > 0) engine->countInFinishedCallback = countInFinished;
...
void BSEngine::timerCallback() {
    if (engineActive) {
        ...
        juce::AudioBuffer<float> *bufferPtr = trackArray.getReference(
                currentTrackNumber)->getCurrentTakeBufferPointer();
        readFile(file, bufferPtr);

        // if the recording was not made from the start, offset it by appropriate number of samples
        if (offsetStartOfRecording) {
            bufferPtr->setSize(bufferPtr->getNumChannels(),
                               bufferPtr->getNumSamples() + offsetNumSamples, true);

            for (int ch = 0; ch < bufferPtr->getNumChannels(); ch++) {
                // move samples from position in buffer to position + offsetNumSamples
                for (int index = bufferPtr->getNumSamples() - 1;
                     index >= offsetNumSamples; index--) {
        ...
        if (file.exists() && bufferPtr->getNumSamples() > 0) {
            if (engine->recordingStoppedCallback) {
                stoppedRecordingCallback();
                engine->recordingStoppedCallback = nullptr;
            }
            engine->stopTimer();
        }
    }
...
void BSEngine::audioDeviceAboutToStart(juce::AudioIODevice *device) {
    ...
    if (device->getCurrentSampleRate() != sampleRate ||
        device->getCurrentBufferSizeSamples() != bufferSize) {
    ...
    std::cout << "inputLatency: " << m_inputLatency << " outputLatency: " << m_outputLatency
              << std::endl;
}
